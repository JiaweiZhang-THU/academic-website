...
这篇论文提出了一种基于多智能体深度强化学习（DRL）的换道算法，旨在提高自动驾驶车辆的安全性、效率、协同性。该算法包含两个层次：上层是一个新颖的多智能体DRL换道决策模型，下层是基于协商的路权分配模型。上层模型通过三个方面来提升决策性能：

1. 利用多智能体强化学习模型来构建车道变换决策问题，提供比单智能体模型更适合协作的范式。
2. 将周围车辆的驾驶意图编码到观测空间中，使得个体车辆能够在决策中隐式协商路权，并以协作的方式确定路权。
3. 设计了一个精巧的奖励函数，使车辆不仅考虑自身的利益，还考虑自车决策对其他车辆的影响，从而引导智能体学习到出色的协同性能。

实验表明，所提出的算法能够引导车辆进行安全、高效和和谐的换道行为，提升车辆之间的协作，进而改善整体交通的安全性和效率。此外，提出的方法促进了车辆的微观同步，这可以导致交通流的宏观同步，对于解决交通拥堵问题至关重要。

论文还讨论了如何通过仿真环境来训练和评估所提出的换道决策规划算法，并对比了不同的模型和算法。通过这些实验，论文验证了所提出策略的性能。
